...
class TimeSeriesDataset(Dataset):
    ...
    def train_test_split_dayofweek(self, train_days: list = [0, 1, 2, 3],
                                   test_days: list = [4], X_weeks=1, Y_weeks=1,
                                   col_names=None):
        ...
        first_day = util.next_weekday(min_date, min(
            all_days))  # 0 = Monday, 1=Tuesday, 2=Wednesday...
        ...
        day_df = self._data.loc[(self._data.index.dayofweek.isin(all_days)) & (
                self._data.index >= first_day) &
                                (self._data.index <= last_day), col_names]

        # self.mldata = day_df.values.astype(np.float32)
        # self.mldata_col_names = col_names

        self.part['train'] = day_df.loc[
            day_df.index.dayofweek.isin(train_days)].values.astype(np.float32)
        self.part['test'] = day_df.loc[
            day_df.index.dayofweek.isin(test_days)].values.astype(np.float32)
    ...
    def train_test_split(self, train_size=0.75, val_size=0):
        import warnings
        warnings.warn('train_test_split is deprecated, use tvt_split instead',
                      DeprecationWarning)

    # n_tx and n_ty are number of x and y timesteps respectively,
    # h is prediction horizon for direct strategy
    # returns x series : {n_tx-1,...,t}, y series : {t+{1 or h},...,t+n_ty-1}
    # x_idx is the list of columns to be used as input or feature
    # y_idx is the list of columns to be used as output or target
    def mvts_to_xy(self, n_tx=1, n_ty=1, x_idx=None, y_idx=None, h=1,
                   ts_list=None):
        ...
        if ts_list is None:
            ...
            if not ts_list:
                raise ValueError(
                    'Timeseries has not been split into train, valid, test.' \
                    + ' Run one of the tvt_split method first')
        else:
            if not self.check_part_list(ts_list):
                raise ValueError(
                    'the part you have asked to split is not available')

        ts_cols = 1 if self.part[ts_list[0]].ndim == 1 else \
            self.part[ts_list[0]].shape[1]
        ...
        #   input series : {t-n_tx,...,t}, output series : {t+1}
        ...
        for ts_part in Dataset.part_all:
            if ts_part not in ts_list:
                ...
                # print(n_rows)
                dataX = np.empty(shape=(n_rows, n_x_vars * n_tx),
                                 dtype=np.float32)
                dataY = np.empty(shape=(n_rows, n_y_vars * n_ty),
                                 dtype=np.float32)
                ...
                for i in range(n_tx, 0, -1):
                    dataX[:, from_col:from_col + n_x_vars] = util.shift(
                        ts[:, x_idx], i)[n_tx:n_rows + n_tx]
                ...
                for i in range(0, n_ty):
                    ...
                    dataY[:, from_col:from_col + n_y_vars] = util.shift(
                        ts[:, y_idx], -(i + h - 1))[n_tx:n_rows + n_tx]
